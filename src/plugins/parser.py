...
class parser():
    ...
    def __init__(self, animeName="", action='d', siteParser=bestanime,
        ...
        self.searchPage = None  # so that you don't have to call it everytime

    # test code for the multi-layering process
    def __setSearchPage(self):
        self.searchPage = self.siteParser.search_page(self.animeName)

    def __getAnimePage(self):
        pass

    # this part here does not work when plugin is multilayered
    def __setEpisodes(self):
        self.episodes = self.siteParser.get_episodes(self.searchPage)

    def __setEpisodeUrls(self):
        self.episodeUrls = self.siteParser.get_episode_url(self.searchPage)
    ...
    def setAnime(self, animeName):
        ...
        self.__setSearchPage()
        if self.siteParser.is_multilayered():
            pass
        self.__setEpisodes()
